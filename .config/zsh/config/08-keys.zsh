...
#* zsh-autocomplete and zsh-edit add many useful keybindings.
...
() {
  ...
  #? Load the more advanced version. -R resolves the function immediately, so we can access the source dir.
  autoload -RUz $1

  #? Load $functions_source, an associative array (a.k.a. dictionary, hash table or map) that maps each function to its source file.
