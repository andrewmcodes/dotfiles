...
() {
  ...
  local gitdir=~/git/plugins  # Where to keep repos and plugins

  # Load all of the files in config that start with <number>- and end in .zsh
  ...
  local __file__=
  for __file__ in $ZDOTDIR/config/<->-*.zsh; do
    . $__file__
  done
} "$@"
